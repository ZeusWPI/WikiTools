...
class ImageLocater(HTMLParser):
    def __init__(self, imagelinks):
        ...
        self.pattern = compile(r'\b((?:https?|ftp).*\.(?:png|jpe?g|gif|bmp))$', M | I)
...
SOURCE_CODE_LOCATION = 'http://zeus.ugent.be/wiki/index.php?title=%s&action=edit'
IMAGE_PATH = 'images/'
...
def get_images(titles):
    h = HTMLParser()
    current_title = 0
    log_file = open(date.today().strftime('%Y-%m-%d %I-%M-%S%p') + '.log', 'wb')
    titles_length = len(titles)
    while current_title < titles_length:
        print('Fetching images from %s... (%s/%s)' % (titles[current_title], current_title + 1, titles_length))
        ''' Escape the title so we can create a valid link'''
        title = titles[current_title].replace('\'','%27').replace(' ', '%20')
        try:
            page = urlopen(SOURCE_CODE_LOCATION % title).read().decode(ENCODING)
        except:
